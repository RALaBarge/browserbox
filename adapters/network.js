...
export const NetworkAdapter = {
  ...
  async start_capture(input) {
    ...
    // Pass pattern as an arg — avoids new Function() in service worker (blocked by extension CSP)
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world:  "MAIN",
      func:   (urlPattern) => {
        if (window.__bb_capturing) return;
        window.__bb_captures = window.__bb_captures || [];
        window.__bb_pattern  = urlPattern;
          ...
          if (shouldCapture(url)) {
            resp.clone().text().then(body => {
              window.__bb_captures.push({
                ts: Date.now(), type: 'fetch', method, url,
                status:      resp.status,
          ...
          xhr.send = function(body) {
            if (shouldCapture(_url)) {
              xhr.addEventListener('loadend', () => {
                window.__bb_captures.push({
                  ts: Date.now(), type: 'xhr', method: _method, url: _url,
                  status: xhr.status, req_headers: {}, res_headers: {},
  ...
  async stop_capture() {
    ...
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      world:  "MAIN",
      func:   () => { window.__bb_capturing = false; window.__bb_pattern = null; },
    });
    return "stopped";
  },
